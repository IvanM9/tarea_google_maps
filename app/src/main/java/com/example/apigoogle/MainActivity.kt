...
class MainActivity : FragmentActivity(), OnMapReadyCallback {
    ...
    fun marcadores(){
        ...
        val peticion = JsonObjectRequest(
            ...
            { response ->
                var str = response.getJSONArray("facultades")

                for (i in 0 until str.length()) {
                    var elemento: JSONObject = str.getJSONObject(i)

                    var parametros: String=""
                    parametros=elemento.getString("carreras")+"&"+
                            elemento.getString("logo")+"&"+
                    ...
                    val marcador = mMap.addMarker(
                        MarkerOptions()
                            ...
                            .title(elemento.getString("Nombre")).snippet(parametros)
                    )
